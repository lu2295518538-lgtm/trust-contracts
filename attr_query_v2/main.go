...
type Entity struct {
	...
	Type       string `json:"type"`       // 实体类型（v2：enterprise/batch/detection_record/...）
	KeyId      string `json:"keyId"`      // 金钥匙（该类型内的唯一标识，如批次号/证号/USCC）
...
func (c *AttrQueryContract) InvokeContract(method string) protogo.Response {
	...
	case "getEntityByUscc":
		return c.getEntityByUscc()
	case "getEntityByKey":
		return c.getEntityByKey()
	default:
...
func (c *AttrQueryContract) saveEntity() protogo.Response {
	...
	// v2 唯一约束：USCC 金钥不可重复（合约层根治 E1/E2 型重复实体）
	if e.Uscc != "" {
		if idb, _ := sdk.Instance.GetStateByte("UCC_"+e.Uscc, "id"); idb != nil {
			return sdk.Error("uscc already registered: " + string(idb))
		}
	}
	// v2 金钥匙锚点：同 (type,keyId) 全局唯一
	if e.Type != "" && e.KeyId != "" {
		kk := "KEY_" + keyHash(e.Type + ":" + e.KeyId)
		if idb, _ := sdk.Instance.GetStateByte(kk, "id"); idb != nil {
			return sdk.Error("entity key already registered: " + string(idb))
		}
	}
	...
	// v2 金钥匙索引
	if e.Type != "" && e.KeyId != "" {
		if err := sdk.Instance.PutStateByte("KEY_"+keyHash(e.Type+":"+e.KeyId), "id", []byte(id)); err != nil {
			return sdk.Error("put key index error: " + err.Error())
		}
	}
...
func (c *AttrQueryContract) matchEntity() protogo.Response {
			...
			if e != nil {
				res := MatchResult{
					...
					Matches: []MatchItem{{
						...
						Scale: e.Scale, Status: e.Status, Type: e.Type,
	...
	// v2 金钥匙精确锚点：type+keyId 直接命中（查询留痕场景）
	if q.Type != "" && q.KeyId != "" {
		if idb, _ := sdk.Instance.GetStateByte("KEY_"+keyHash(q.Type+":"+q.KeyId), "id"); idb != nil {
			e := c.loadEntity(string(idb))
			if e != nil {
				res := MatchResult{
					Query: qj, QueryTokens: 1, Candidates: 1, TopK: k,
	...
	for _, id := range cands {
		...
		if q.Type != "" && e.Type != "" && e.Type != q.Type {
			continue // v2：类型过滤
		}
		s := stats[id]
		h := s.exact
...
// getEntityByUscc 按统一社会信用代码精确查询（v2，只读）
func (c *AttrQueryContract) getEntityByUscc() protogo.Response {
	args := sdk.Instance.GetArgs()
	uscc := string(args["uscc"])
	if uscc == "" {
		return sdk.Error("uscc is required")
	}
	idb, _ := sdk.Instance.GetStateByte("UCC_"+uscc, "id")
	if idb == nil {
		return sdk.Success([]byte("{}"))
	}
	raw, err := sdk.Instance.GetStateByte("ENT_"+string(idb), "data")
	if err != nil || raw == nil {
		return sdk.Success([]byte("{}"))
	}
	return sdk.Success(raw)
...
// getEntityByKey 按 (type,keyId) 金钥匙精确查询（v2，只读）
func (c *AttrQueryContract) getEntityByKey() protogo.Response {
	args := sdk.Instance.GetArgs()
	typ := string(args["type"])
	kid := string(args["keyId"])
	if typ == "" || kid == "" {
		return sdk.Error("type and keyId are required")
	}
	idb, _ := sdk.Instance.GetStateByte("KEY_"+keyHash(typ+":"+kid), "id")
	if idb == nil {
		return sdk.Success([]byte("{}"))
	}
	raw, err := sdk.Instance.GetStateByte("ENT_"+string(idb), "data")
...
func tokensOf(e *Entity) []string {
	...
	// v2.1 修复：空属性不生成占位 token（旧逻辑使所有空值实体共享 "u="/"t=" 等桶 → 候选噪声）
	if e.Uscc != "" {
		add("u=" + e.Uscc)                     // 信用代码：精确
	}
	if e.Type != "" && e.KeyId != "" {
		add("k=" + e.Type + ":" + e.KeyId)     // v2 金钥匙：(类型,标识) 精确
	}
	if e.BizType != "" {
